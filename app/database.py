...
async def init_db():
    ...
    if os.getenv("MOCK_SERVICES") == "1":
        logger.info("MOCK_SERVICES is set, bypassing PostgreSQL database init.")
        return
    ...
    for i in range(max_retries):
        try:
            _pool = await asyncpg.create_pool(
                os.getenv(
                    "DATABASE_URL",
                    "postgresql://postgres:postgres@localhost:5432/ratesentry",
                ),
                min_size=2,
                max_size=10,
            )
            ...
            if i == max_retries - 1:
                raise e
            logger.warning(
                f"Database not ready, retrying in 2s... ({i + 1}/{max_retries})"
            )
